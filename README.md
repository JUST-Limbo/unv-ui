...
比如我想要一个省市区选择器：它可以是弹窗形式，也可以是并列三连的选择器，也可以是级联选择器。从功能上讲他们都是省市区选择器，然而他们的UI是不同的，开发者在组织代码时可能会出现`areaV1.vue、areaV2.vue、areaV3.vue`，但是他们的逻辑是相通的。数据兼容层组件可以让开发者更专注于实现组件的视图部分。
